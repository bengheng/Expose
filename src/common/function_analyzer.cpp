...
bool function_analyzer::add_call (ea_t ea)
{
	...
	//qsnprintf(pLMLog->GetBuffer(), pLMLog->GetBufferSize(), "FA_DEBUG> [%d] add_call ea_start=%08x startEA=%08x (%08x, %08x)\n", num_calls, ea_start, get_func(ea)->startEA, calls_src[num_calls], calls_dst[num_calls]);
...
bool function_analyzer::disasm (ea_t ea, char *disasm_buf)
{
    ...
    ua_outop2(ea, op_buf, 512, op_num++, GETN_NODUMMY);
...
void function_analyzer::gml_export (char *file_path, int base_node_id)
{
    ...
    get_root_filename(rootfname, sizeof(rootfname)-1);

    // construct the graph output file name as "file path + get_root_filename() + '-' + function address + .gml".
    qsnprintf(path, sizeof(path) - 1, "%s\\%s-%08x.gml", file_path, rootfname, ea_start);
    ...
    qsnprintf(path, sizeof(path) - 1, "%s\\%s.xrefs", file_path, rootfname);
    ...
    for (i = 1; i <= num_nodes; i++)
    {
        ...
        for (ea = nodes[i]; ea != BADADDR; ea = next_ea(ea))
        {
            ...
            for (len = 0, dis = disasm_buf ; len < strlen(disasm_buf); len += amount)
            {
                ...
                qsnprintf(gml_line, amount, "%s", dis);
...
Finds the node for the input address.
*/
ea_t function_analyzer::get_node(ea_t addr)
{
	while(addr != BADADDR)
	{
		if(is_node(addr)) return addr;
		addr = prev_ea(addr);
	}
