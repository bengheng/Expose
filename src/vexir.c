...
static Bool
chase_into_ok (void *closureV, Addr64 addr64)
{
  return False;
}
...
//----------------------------------------------------------------------
static IRSB *
instrument1 (void *callback_opaque,
	     IRSB * irbb,
	     VexGuestLayout * vgl,
	     VexGuestExtents * vge, IRType gWordTy, IRType hWordTy)
{
  ...
  printf("instrument1\tDEBUG0 irbb_current->stmts_used %d irbb_current->stmts_size %d\n",
	irbb_current->stmts_used,
	irbb_current->stmts_size);
...
translate_init ()
{
  ...
  vc.guest_max_insns = 80;	// By default, we translate 1 instruction at a time
  ...
  vta.traceflags = (1 << 5) | (1 << 2 );		// Debug verbosity
...
//----------------------------------------------------------------------
IRSB *
translate_insn (VexArch guest,
		unsigned char *insn_start, unsigned int insn_addr)
{
  ...
  printf("translate_insn: irbb_current->stmts_used: %d\n", irbb_current->stmts_used);
  printf("translate_insn: irbb_current->stmts_size: %d\n", irbb_current->stmts_size);
