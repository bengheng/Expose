...
unsigned char * BitVector_Error(ErrCode error) {
    ...
    case ErrCode_Ok:
        ...
        break;
    case ErrCode_Type:
        return( (unsigned char *) ERRCODE_TYPE );
        break;
    case ErrCode_Bits:
        return( (unsigned char *) ERRCODE_BITS );
        break;
    case ErrCode_Word:
        return( (unsigned char *) ERRCODE_WORD );
        break;
    case ErrCode_Long:
        return( (unsigned char *) ERRCODE_LONG );
        break;
    case ErrCode_Powr:
        return( (unsigned char *) ERRCODE_POWR );
        break;
    case ErrCode_Loga:
        return( (unsigned char *) ERRCODE_LOGA );
        break;
    case ErrCode_Null:
        return( (unsigned char *) ERRCODE_NULL );
        break;
    case ErrCode_Indx:
        return( (unsigned char *) ERRCODE_INDX );
        break;
    case ErrCode_Ordr:
        return( (unsigned char *) ERRCODE_ORDR );
        break;
    case ErrCode_Size:
        return( (unsigned char *) ERRCODE_SIZE );
        break;
    case ErrCode_Pars:
        return( (unsigned char *) ERRCODE_PARS );
        break;
    case ErrCode_Ovfl:
        return( (unsigned char *) ERRCODE_OVFL );
        break;
    case ErrCode_Same:
        return( (unsigned char *) ERRCODE_SAME );
        break;
    case ErrCode_Expo:
        return( (unsigned char *) ERRCODE_EXPO );
        break;
    case ErrCode_Zero:
        return( (unsigned char *) ERRCODE_ZERO );
        break;
    default:
        return( (unsigned char *) ERRCODE_OOPS );
        break;
    }
}
...
unsigned int BitVector_Mask(unsigned int bits)           /* bit vector mask (unused bits) */
{
    ...
    mask = bits & MODMASK;
    if (mask) mask = (unsigned int) ~(~0L << mask);
    else mask = (unsigned int) ~0L;
    return(mask);
}
...
signed int BitVector_Lexicompare(unsigned int *  X, unsigned int *  Y) {
    ...
    if (bitsX == bitsY) {
        ...
        if (r) return((signed int) 0);
            ...
            if (*X < *Y) return((signed int) -1);
            else return((signed int) 1);
        }
    }
    else {
        if (bitsX < bitsY) return((signed int) -1);
        else return((signed int) 1);
    }
}
...
unsigned char * BitVector_to_Dec(unsigned int *  addr)
{
    ...
    if ((bits < 4) || (sign == 0))
    {
        if (bits > 0) digits = *addr;
        else digits = (unsigned int) 0;
...
ErrCode BitVector_Multiply(unsigned int *  X, unsigned int *  Y, unsigned int *  Z)
{
    ...
    */
    ...
    else
    {
        ...
        sgn_x = sgn_y ^ sgn_z;
        if (sgn_y) BitVector_Negate(A,Y);
        else BitVector_Copy(A,Y);
        if (sgn_z) BitVector_Negate(B,Z);
...
ErrCode BitVector_Divide(unsigned int *  Q, unsigned int *  X, unsigned int *  Y, unsigned int *  R)
{
    ...
    */
    ...
    else
    {
        ...
        sgn_q = sgn_x ^ sgn_y;
        if (sgn_x) BitVector_Negate(A,X);
        else BitVector_Copy(A,X);
        if (sgn_y) BitVector_Negate(B,Y);
        else BitVector_Copy(B,Y);
...
ErrCode BitVector_GCD(unsigned int *  X, unsigned int *  Y, unsigned int *  Z)
{
    ...
    */
    ...
    if (sgn_a) BitVector_Negate(A,Y);
    else BitVector_Copy(A,Y);
    if (sgn_b) BitVector_Negate(B,Z);
    else BitVector_Copy(B,Z);
    while (! error)
    {
        if (! (error = BitVector_Div_Pos(Q,A,B,R)))
    ...
    if (! error)
    {
        if (sgn_b) BitVector_Negate(X,B);
        else BitVector_Copy(X,B);
    }
    BitVector_Destroy(Q);
    BitVector_Destroy(R);
    BitVector_Destroy(A);
...
ErrCode BitVector_GCD2(unsigned int *  U, unsigned int *  V, unsigned int *  W, unsigned int *  X, unsigned int *  Y)
{
    ...
    */
    ...
    if (sgn_a) BitVector_Negate(A,X);
    else BitVector_Copy(A,X);
    if (sgn_b) BitVector_Negate(B,Y);
    else BitVector_Copy(B,Y);
    BitVector_Empty(X1);
    BitVector_Empty(X2);
    *X1 = 1;
    ...
    *Y2 = 1;
    ...
    while (! error)
    {
        ...
        if (sgn_x) BitVector_Negate(Z,X2);
        else BitVector_Copy(Z,X2);
        ...
        if (sgn_y) BitVector_Negate(Z,Y2);
        else BitVector_Copy(Z,Y2);
    ...
    if (! error)
    {
        if (sgn_b) BitVector_Negate(U,B);
        else BitVector_Copy(U,B);
        BitVector_Copy(V,X2);
        BitVector_Copy(W,Y2);
    }
...
void BitVector_Block_Store(unsigned int *  addr, unsigned char * buffer, unsigned int length)
{
    ...
    if (size > 0)
    {
        while (size-- > 0)
        {
            ...
            {
                value |= (((unsigned int) *buffer++) << count);
                length--;
            }
            *addr++ = value;
        }
