...
#define VARNAMESIZE ((size_t) 20)
#define NAMELESS_HASH ((uint32_t) 0xffffffff)
...
vector< set< pair<uint32_t, nodestar> >* > varLUT;
...
Expr vc_readExpr(VC vc, Expr array, Expr index) {
	...
	vc_addVarByHash( NAMELESS_HASH, output );
	return output;
}

// //! Array update; equivalent to "array WITH [index] := newValue"
Expr vc_writeExpr(VC vc, const char *name, Expr array, Expr index, Expr newValue) {
	bmstar b = (bmstar)(((stpstar)vc)->bm);
	nodestar a = (nodestar)array;
	nodestar i = (nodestar)index;
...
void vc_push(VC vc) {
	...
	varLUT.push_back( new set< pair<uint32_t, nodestar> >() );
}
...
void vc_pop(VC vc) {
	...
	set< pair<uint32_t, nodestar> > *s;
	if( varLUT.size() == 0 ) return;

	s = varLUT.back();
	varLUT.pop_back();

	// Delete all Expr in s
	while( s->size() > 0 )
	{
		vc_DeleteExpr( s->begin()->second );
		s->erase( s->begin() );
	}
...
Expr vc_trueExpr(VC vc) {
	...
	vc_addVarByHash( NAMELESS_HASH, d );
	return d;
}
...
Expr vc_bvWriteToMemoryArray(VC vc, const char *name,
	...
	if (numOfBytes == 1)
		return vc_writeExpr(vc, name, array, byteIndex, element);
	else {
		int count = 1;
		...
		Expr newarray = vc_writeExpr(vc, name, array, byteIndex, c);
		while (--numOfBytes > 0) {
			hi = low-1;
			low = low-8;
...
void vc_Destroy(VC vc) {
	...
	//	delete aaa;
	//}	
	vector< set< pair<uint32_t, nodestar> >* >::iterator b1, e1;
	for( b1 = varLUT.begin(), e1 = varLUT.end();
	b1 != e1; ++b1 )
	{
		set< pair<uint32_t, nodestar> > *curset = *b1;

		set< pair<uint32_t, nodestar> >::iterator b2, e2;
		for( b2 = curset->begin(), e2 = curset->end();
		b2 != e2; ++b2 )
		{
			delete b2->second;
		}
...
/*!
We need to hash the contents of the string because it seems
that map is using the address of the name string.
*/
void vc_addVarByName( const char *name, const Expr e )
{
	uint32_t hash;

	MurmurHash3_x86_32( name, strlen(name), 0, &hash );
	vc_addVarByHash( hash, e );
}

/*!
Adds expression by hash.
*/
void vc_addVarByHash( const uint32_t hash, const Expr e )
{
	vector< set< pair<uint32_t, nodestar> >* >::iterator vitr;
	set< pair<uint32_t, nodestar> > *s;

	if ( hash != NAMELESS_HASH &&
	        vc_lookupVarByHash( hash ) != NULL )
	{
		printf("Hash %08x already exists!\n", hash);
		assert( vc_lookupVarByHash(hash) == NULL );
		return;
	}

	// Get the last set
	vitr = varLUT.end();
	if ( vitr == varLUT.begin() )
	{
		s = new set< pair<uint32_t, nodestar> >();
		varLUT.push_back( s );
	}
	else
	{
	...
	s->insert( make_pair( hash, (nodestar)e ) );
}

/*!
Looks up the variable by name.
*/
Expr vc_lookupVarByName( const char *name, ... )
{
	...
	va_start( args, name );
	vsnprintf( varname, VARNAMESIZE, name, args );
	va_end( args );

	MurmurHash3_x86_32( varname, strlen(varname), 0, &hash );
	return vc_lookupVarByHash( hash );
}

/*!
Looks up the variable by hash.
*/
Expr vc_lookupVarByHash( const uint32_t hash )
{
	vector< set< pair<uint32_t, nodestar> >* >::iterator	vitr_b, vitr_e;
	set< pair<uint32_t, nodestar> >				*curset;
	set< pair<uint32_t, nodestar> >::iterator			sitr_b, sitr_e;

	for ( vitr_b = varLUT.begin(), vitr_e = varLUT.end();
	        vitr_b != vitr_e; ++vitr_b )
	{
		curset = *vitr_b;
		for ( sitr_b = curset->begin(), sitr_e = curset->end();
		        sitr_b != sitr_e; ++sitr_b )
		{
			if ( hash == (*sitr_b).first ) // found
				return (*sitr_b).second;
		}
	}
